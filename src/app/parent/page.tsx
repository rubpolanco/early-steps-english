...
export default async function ParentHomePage() {
  ...
  const s = t.parentApp.home;

  return (
    <div>
      <SectionTitle>{s.welcomeBack}, {session.name.split(" ")[0]}! 👋</SectionTitle>
              ...
              <div key={child.id} className="card p-5">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-brand-navy/60">{calcAge(child.dob, locale)} · {classroom?.name ?? s.unassigned}</p>
                  </div>
                ...
                {attendance?.check_in_time && (
                  <p className="text-xs text-brand-navy/70 mb-1">
                    {s.checkedInAt} {attendance.check_in_time.slice(11, 16)} {s.by} {attendance.check_in_by_name}
                  </p>
                )}
                {attendance?.check_out_time && (
                  <p className="text-xs text-brand-navy/70 mb-3">
                    {s.pickedUpAt} {attendance.check_out_time.slice(11, 16)} {s.by} {attendance.check_out_by_name}
                  </p>
                )}
                {report ? (
                  <p className="text-xs bg-brand-sky-light rounded-lg p-2 text-brand-navy/80 mb-3">
                    {s.moodToday}: <strong>{report.mood}</strong>
                  </p>
                ) : (
                  <p className="text-xs text-brand-navy/50 mb-3">{s.noReportYet}</p>
                )}
                <div className="flex gap-2 flex-wrap text-xs">
                  <Link href="/parent/reports" className="btn-secondary px-3 py-1.5">{t.nav.dailyReports}</Link>
                  <Link href="/parent/photos" className="btn-secondary px-3 py-1.5">{t.nav.photos}</Link>
                  <Link href="/parent/pickup-people" className="btn-secondary px-3 py-1.5">{t.nav.pickupPeople}</Link>
                </div>
              </div>
            );
          })}
